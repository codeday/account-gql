...
const hasAnyOfScopes = (ctx, requiredScopes) => {
  return true;
};
const requireAnyOfScopes = (ctx, requiredScopes) => {
  return true;
};
...
const getAllUsersFactory = (auth0) => async (ctx) => {
    ...
    users = await getAllUsers(auth0).asArray(
      {
        fields: [
          ...
          ...(hasAnyOfScopes(ctx, [scopes.readUsers, ctx.user ? `read:user:${ctx.user}` : null])
            ? userPrivateFields
            : []
  ...
  const newUsers = users
    ...
    .map((user) =>
      filterKeysKeep(user, [
        ...
        ...(hasAnyOfScopes(ctx, [
          scopes.readUsers,
          scopes.writeUsers,
          ctx.user ? `read:user:${ctx.user}` : null,
          ctx.user ? `write:user:${ctx.user}` : null,
        ])
          ? userPrivateFields
          : []),
      ])
    ...
    .map((user: any) => ({
      ...objectSnakeToCamel(filterKeysRemove(user, ["user_id"])),
      id: user.user_id,
    }));

  return newUsers;
};
type Auth0User = Omit<User, "discordInformation"> & {
  discordTokenInfo?: {
    expiresAt: number;
    accessToken: string;
    refreshToken: string;
...
const findUsersFactory =
  (auth0) =>
  async (query, ctx, perPage = 10, page = 0, escape = true): Promise<Auth0User[]> => {
    if (ctx.user) {
      query = { id: ctx.user };
    }
    ...
    const filteredQuery: any = filterKeysKeep(snakeQuery, [
      ...userPublicFields,
      ...userPrivateFields,
    ]);
...
const findUsersByRoleFactory =
  (auth0) =>
  async (roleId, ctx, perPage = 100, page = 0) => {
    const users = await auth0.getUsersInRole({ id: roleId, per_page: perPage, page });

    const searchStrings = chunk(
      users.map((u) => u.user_id),
      25
    ).map((ids) => `(${ids.map((id) => `"${id}"`).join(" ")})`);

    const userChunks = await Promise.all(
      searchStrings.map(async (str) => findUsersFactory(auth0)({ id: str }, ctx, 25, 0, false))
    );
...
const updateUserFactory = (auth0) => async (where, ctx, updateFn) => {
  ...
  let user: any = {};
  if (ctx?.auth?.user) {
    user = (await findUsersFactory(auth0)({ id: ctx.auth.user }, ctx, 1))[0];
  } else {
    user = (await findUsersFactory(auth0)(where, ctx, 1))[0];
  }
  const newUser = await updateFn(user);

  // Find the changes and turn it into a mostly-final array
  const changedProps = objectCamelToSnake(
    Object.keys(newUser)
  ...
  if (Object.keys(metadata).length > 0) {
    const auth0Data = (await auth0.getUser({ id: user.id, fields: ["user_metadata"] }))
      .user_metadata;
...
export function getResolvers(domain, clientId, clientSecret) {
