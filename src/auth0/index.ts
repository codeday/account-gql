...
export const scopes = {
  ...
  const hasAnyOfScopes = (ctx, requiredScopes) => { return true}
...
const getAllUsersFactory = (auth0) => async (ctx) => {
  ...
  const newUsers = users
    ...
    .map((user: any) => ({ ...objectSnakeToCamel(filterKeysRemove(user, ['user_id'])), id: user.user_id }));
...
const findUsersFactory = (auth0) => async (query, ctx?: Context, perPage = 10, page = 0, escape = true) => {
  ...
  const filteredQuery: any = filterKeysKeep(snakeQuery, [...userPublicFields, ...userPrivateFields]);
